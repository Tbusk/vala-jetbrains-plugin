...
In order to add and modify keywords, a few steps are necessary.

1. Open `src/main/java/com/tbusk/vala_plugin/syntax/Vala.bnf`. This file is necessary for defining keywords and handling syntax checks.  
2. Right-click the `Vala.bnf` file and select `Generate Parser Code`. This will generate the necessary parser code files in `src/main/gen/`.
3. Open `src/main/java/com/tbusk/vala_plugin/ValaTokenSets.java` and add the keyword to `KEYWORDS_STRINGS` and `KEYWORDS`, or whichever is applicable.
4. If it is a new type, then you'll need to define the type in `src/main/java/com/tbusk/vala_plugin/highlighting/ValaSyntaxHighlighter.java`.
5. Open `src/main/java/com/tbusk/vala_plugin/syntax/Vala.flex` and add the keyword there.
6. Right-click the `Vala.flex` file and select `Run JFlex Generator`. This will generate the necessary lexer code files in `src/main/gen/`.
7. Test your changes by running the Gradle task `runIde`.
